...
Launches `sim_ros`, the plan node (`mpc_ros` by default), and TF helpers:
...
## Configuration
Configuration files are located in `mm_run/config/` (robot, scene, controller, simulation) and `mm_rl/config/` (RL training).
